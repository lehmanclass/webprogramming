...
router.post('/register', (req, res) => {
  ...
  User.findOrCreate({
    ...
    .then(([data, created]) => {
      ...
      const todaySection = new Section({
        userId: user.id,
        name: 'today'
      });
      
      const weekSection = new Section({
        userId: user.id,
        name: 'week'
      });
      ...
      todaySection.save()
        .then(() => {
          weekSection.save()
            ...
            .catch(() => { weekSection.destroy(); res.status(500).json({msg: 'Error occured authenticating user'}); });
        })
        .catch(() => { todaySection.destroy(); res.status(500).json({msg: 'Error occured authenticating user'}); });
